...
在对应楼层探索时，有 40% 概率遇到 NPC 角色。安可欣、安继年、Luna、洛疏律在 Level 1（Alpha 基地）出现，白宇在 Level 2（管道迷宫）出现，祁子午在 Level 3（电气站）出现。每次遭遇会自动增加好感度，达到阈值后可邀请角色同行。

- **好感度获取**：每次遭遇角色自动增加好感度（默认 2 点）
- **赠礼提升**：使用 `/br gift <角色名> <编号>` 赠送背包物品可额外提升好感度
- **邀请同行**：好感度达到阈值（默认 70）后，使用 `/br invite <角色名>` 邀请角色一起探索
- **同行加成**：同行角色提供 **+5% 出口率**加成，探索时触发专属互动台词
- **解散同行**：使用 `/br dismiss` 让同行角色返回 Alpha 基地

### 赠礼系统

所有物品赠礼增加的好感度相同，可在 `config.toml` 的 `gift_favorability_values` 中自定义：
...
│   ├── qiziwu.txt              ← 祁子午（Level 3 通讯技术员，7段剧情）
└── ...
```
